(function ($) {
  ...
  jQuery(document).ready(function ($) {
    ...
    // counter-section
    //-------------------------------------------------------
    $(".counter-section2").on("inview", function (
      event,
      visible,
      visiblePartX,
      visiblePartY
    ...
    function checkSubscriptionStatus(invoiceId) {
      var _this = $(this);
      var targetForm = _this.closest("form");
      var errroTarget = targetForm.find(".response");
      if (invoiceId !== 0) {
        $.ajax({
          method: "get",
          url: "check-subscription-status/" + invoiceId,
          // data:formDetail,
          cache: false,
          contentType: false,
          processData: false,
          ...
          if (resp && resp.success) {
            errroTarget.html(
              '<p style="color:green;">' + resp.success + "</p>"
            );
          } else {
            errroTarget.html(
